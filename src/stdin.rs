...
/// Ctrl+D never did anything once BusyBox's own line editor was driving the prompt (effectively
/// always, interactively) — found live via a synthetic-keystroke QEMU-monitor `sendkey` test.
/// Every other `c_cc` slot (`VEOL`/`VEOL2`/`VSWTC`) stays real-default `0` ("unset"), matching a
/// real fresh tty.
