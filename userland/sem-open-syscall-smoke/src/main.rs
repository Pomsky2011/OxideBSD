...
//! fixture, seeded by `modules/oxfs`'s `format_fresh_filesystem` -- see `build.rs`'s
//! `build_sem_open_smoke` and `userland/sem-open-smoke/main.c`'s own doc comment) actually runs to
//! completion. This is the scenario `process::limits::futex_key` (`src/process/limits.rs`) exists
...
//! clean exit -- exactly the same shape `userland/pthread-syscall-smoke/` already established for
