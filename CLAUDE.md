...
| `/proc` filesystem | not started | 26 (`NEEDS_PROC`) | `ps`/`top`/`pgrep`/`pkill`/`pidof`/`lsof`/`free`/`fuser`/... all read it; would need per-process bookkeeping exposed as a real (pseudo-)filesystem, not just more `modules/oxfs` syscalls |
